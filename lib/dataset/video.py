...
def vis_X(X,  save_folder, id):
    ...
    for jb in range(n):
        file_ID = f"{id}_b{jb}"
        T.ToPILImage()(X[jb]).save(f'{save_folder}/{file_ID}_MDM.png', mode='png')

def load_image_f( directory, file_frame):
    try:
        X = T.ToTensor()(Image.open(f'{directory}/{file_frame}').convert('RGB'))
    except Exception as e:
        print('error loading image:', f'{directory}/{file_frame}')
        exit()
    return X
...
def augmentation_video( X , scales , img_output_size, param):
    ...
    if param["ColorJitter"]: transform_list.append(T.ColorJitter(brightness=(0.5, 1.5), contrast=(0.5, 1.5), saturation=(0.5, 1.5), hue=0.5))
...
def size_i_add_from_record ( record, fps, t_length):
    ...
    if t_length > 0:
        ...
        if last_i > record["imagefolder_size"]:
            ...
            if size < 10:
                print("size_i_add_from_record:", size, record["imagefolder"], t_start, t_length, record["imagefolder_size"])
                exit()
    return size , i_add
...
def sample_indices_training( size, num_segments, k_frames, i_add ):
    ...
    indices = np.array([int(tick / 2.0 + tick * x + rv) + randint(-average_duration, average_duration+1) for x in range(num_segments)])
...
def sample_indices_validation(size, num_segments, k_frames, i_add ):
    tick = size  / float(num_segments)
    indices = np.array([int(tick / 2.0 + tick * x) for x in range(num_segments)])
    indices[indices < k_frames//2] = k_frames//2
    indices[indices > size -1 - k_frames // 2] = size - (k_frames // 2) - 1
    return indices  + 1+ i_add
...
def get_video_x(record, args,  mode_train_val):
    ...
    if mode_train_val == "training":
        ...
        #indices = sample_indices_stride(size, num_segments, k_frames, frame_stride)
        indices = sample_indices_validation(size, num_segments, k_frames , i_add)
    else:
        indices = sample_indices_validation(size, num_segments, k_frames, i_add)
    ...
    if mode_train_val == "training":
        X = augmentation_video(X, video_augmentation_param["scales"], video_img_param["img_output_size"], video_augmentation_param)
    elif mode_train_val == "validation":
        X = validation_transform( X , video_img_param["img_input_size"], video_img_param["img_output_size"])
    else:
        X = validation_transform(X, video_img_param["img_input_size"], video_img_param["img_output_size"])
    #print("X", X.size())
