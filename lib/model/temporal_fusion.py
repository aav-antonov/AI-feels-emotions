...
class TempoModalShift(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.mode == "shift_temporal":
            ...
            raise ValueError(f'TempoModalShift Unknown mode: {self.mode} (must be "shift_temporal", "shift_single_modality" or "shift_temporal_modality")')
...
def shift_block(stage, n_video_segments, n_motion_segments, n_audio_segments, input_mode,  f_div,  shift_depth, mode , n_insert, m_insert):
    ...
    for i, b in enumerate(blocks):
        if i % n_insert == m_insert:
            #print(f'shift_block:  stage with {len(blocks)} blocks ,mode:{mode}')
            blocks[i].conv1 = TempoModalShift(b.conv1, n_video_segments, n_motion_segments, n_audio_segments, input_mode, f_div,shift_depth, mode)
...
def make_Shift(net, n_video_segments, n_motion_segments, n_audio_segments,  input_mode,  f_div,  shift_depth, mode , n_insert, m_insert):
    ...
    if isinstance(net, (torchvision.models.ResNet,timm.models.ResNet)):
        ...
        net.layer1 = shift_block(net.layer1, n_video_segments, n_motion_segments, n_audio_segments,  input_mode,  f_div,  shift_depth, mode,n_insert, m_insert)
        net.layer2 = shift_block(net.layer2, n_video_segments, n_motion_segments, n_audio_segments,  input_mode,  f_div,  shift_depth, mode,n_insert, m_insert)
        net.layer3 = shift_block(net.layer3, n_video_segments, n_motion_segments, n_audio_segments,  input_mode,  f_div,  shift_depth, mode,n_insert, m_insert)
        net.layer4 = shift_block(net.layer4, n_video_segments, n_motion_segments, n_audio_segments,  input_mode,  f_div,  shift_depth, mode,n_insert, m_insert)

    elif isinstance(net, torchvision.models.DenseNet):

        net.features.denseblock1 = shift_block(net.features.denseblock1, n_video_segments, n_motion_segments, n_audio_segments, input_mode,  f_div,  shift_depth, mode,n_insert, m_insert)
        net.features.denseblock2 = shift_block(net.features.denseblock2, n_video_segments, n_motion_segments, n_audio_segments, input_mode,  f_div,  shift_depth, mode,n_insert, m_insert)
        net.features.denseblock3 = shift_block(net.features.denseblock3, n_video_segments, n_motion_segments, n_audio_segments, input_mode,  f_div,  shift_depth, mode,n_insert, m_insert)
        net.features.denseblock4 = shift_block(net.features.denseblock4, n_video_segments, n_motion_segments, n_audio_segments, input_mode,  f_div,  shift_depth, mode,n_insert, m_insert)
